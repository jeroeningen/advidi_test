...
class Banner < ActiveRecord::Base
  ...
  def add_banner_to_redis
    if campaign.present?
      ...
      # Production:
      # Heroku uses S3, so for production set the image URL as path
      Banner.paths[self.id] = case
        when $environment == :production then image.url
        else "#{Dir.pwd}/public/#{image.store_dir}/#{image.file.present? ? image.file.filename : image.filename}"
      end
    end
  end
